...
lbls <- paste(c('OLS','Ridge','LASSO'),' AUC=',c(v1,v2,v3),sep=' ')
par(mfrow=c(1,1))
jpeg('AUCbyModel.jpeg',width=960,height=960)
plot(prf1, colorize = FALSE,col='blue',main='Model Performance by Regression Method \n (Hold Out)')
plot(prf2, add = TRUE, colorize = FALSE,col='red')
plot(prf3, add = TRUE, colorize = FALSE,col='green')
grid(5,5,'gray44');abline(0,1,col='black')
legend('bottomright',lbls,col=c('blue','red','green'),lty=c(1,1,1))
dev.off()
